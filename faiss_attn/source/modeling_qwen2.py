...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: Optional[int] = None):
        ...
        if (self.head_dim * self.num_heads) != self.hidden_size and "Qwen3ForCausalLM" not in self.config.architectures:
            raise ValueError(
                f"hidden_size must be divisible by num_heads (got `hidden_size`: {self.hidden_size}"
                f" and `num_heads`: {self.num_heads})."
            )
        attention_bias = True if "Qwen2ForCausalLM" in self.config.architectures else config.attention_bias
        ...
        if "Qwen3ForCausalLM" in self.config.architectures:
